...
const getUserName = () => {
  if (gameInfo.userName !== null) {
    gameInfo.userWannaPlay = !confirm(
      `¿Quieres seguir jugando con el usuario ${gameInfo.userName}?`
    );
  }
...
const showRules = () => {
  ...
  console.log(`Bienvenido ${
    gameInfo.userName
  }, el objetivo principal del juego es adivinar la palabra que se esconde detras de cada definición.
...
const startGame = () => {
  ...
  while (
    ...
    gameInfo.wordsAnswered != questions.length
  ) {
    runRound();
  }
...
const manageAnswer = () => {
  if (/* gameInfo.userAnswer == '' || */ gameInfo.userAnswer == "pasapalabra") {
    ...
    gameInfo.wordsAnswered += 1;
    alert(
      `No es correcto, la palabra era ${questions[letter].answer.toUpperCase()}`
    );
  }
};
