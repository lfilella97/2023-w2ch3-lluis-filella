const textoInferiorPantalla = document.querySelector(".textoInferiorPantalla");
const textoSuperiorPantalla = document.querySelector(".textoSuperiorPantalla");
...
clearEntryButton.addEventListener("click", () => {
  ...
  refreshScreens(
    ...
    Number(calculator.provisionalNumber.join(""))
  );
});
...
const fixNumber = () => {
  if (!Number.isInteger(calculator.result)) {
    calculator.result = Number(calculator.result).toFixed(2);
  }
};
...
const showOnSuperiorScreen = (superior) => {
  if (Number.isFinite) {
    if (superior.toString().split("").length > 16) {
      superior =
        superior.toString().split("")[0] +
        "x10^" +
        superior.toString().split("").length;
    }
  }
...
const showOnInferiorScreen = (inferior) => {
  if (Number.isFinite) {
    if (inferior.toString().split("").length > 11) {
      inferior =
        inferior.toString().split("")[0] +
        "x10^" +
        inferior.toString().split("").length;
    }
  }
...
const getResult = (sign) => {
  switch (sign) {
    ...
    case "invert":
      ...
      break;
    case undefined:
      calculator.result = Number(calculator.number);
      break;
    default:
      break;
  }
...
const plus = () => {
  calculator.result = Number(calculator.result) + Number(calculator.number);
  fixNumber();
